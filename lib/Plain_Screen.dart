...
class _PageState extends State<Page> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    super.initState();
    keys = List.generate(
        2 + widget.answers.length, (_) => GlobalKey<_ItemFaderState>());
  ...
  Future<void> animatedDot(Offset startOffset) async {
  ...
  void onTap(int keyIndex, Offset offset) async {
    for (GlobalKey<_ItemFaderState> key in keys) {
      ...
      if (keys.indexOf(key) == keyIndex) {
        ...
        animatedDot(offset).then((_) => widget.onOptionSelected());
      }
    }
  }
    ...
    return AnimatedBuilder(
      ...
      builder: (context, child) {
        return Transform.translate(
          offset: Offset(
            0,
            64 * position * (1 - _animationController.value),
          ),
          child: Opacity(
